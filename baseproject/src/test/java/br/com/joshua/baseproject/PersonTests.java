...
class PersonTests {
	...
	private Person personRequest = null;
	...
	public void should_savePerson_when_use_service() {
		...
		GetPersonResponse responseDto = gateway.savePerson(personRequest);
	...
	public void should_savePerson_when_use_repository() {
		...
		when(repository.save(any(PersonModel.class))).then(new Answer<PersonModel>() {
		...
		PersonModel response = repository.save(person);
	...
	public void should_deletePerson_when_use_repository() {
		Optional<PersonModel> optionalPerson = Optional.of(person);
	...
	@Test
	public void should_findPerson_when_use_gateway() {

		when(gateway.findOnePerson(1L)).thenReturn(personResponse);

		GetPersonResponse personReturnDto = gateway.findOnePerson(1L);

		verify(gateway, times(1)).findOnePerson(1L);
	...
	public void should_findPerson_when_use_repository() {
		...
		Optional<PersonModel> personReturnOptional = repository.findById(1L);
		PersonModel personReturn = personReturnOptional.get();
